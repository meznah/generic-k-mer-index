...
unsigned int  GLOBAL_SEED = 2;

KmerType  init() {
    UI k =32, j;
    std::cout<< "GLOBAL_SEED = " << GLOBAL_SEED << std::endl;
    srand(GLOBAL_SEED);
    KmerType  h = 0 ;
    for (UI i=0; i < k+1; i++) {
         j= rand() % 4;  //pick randomly A, C, G, or T
         h = h << 2; h |= j;  
        } 
  return h;
}

KmerType  GLOBAL_KMER = init();
...
//KmerType  _hash(char * kmer, const unsigned int k)
{
   ...
   //add randomization step
   //auto GLOBAL_KMER_temp = (GLOBAL_KMER >> (64-k));
   KmerType temp =  h ^ GLOBAL_KMER;
   h = temp;  
   //done from randomization.  
...
{
   //remove randomization step
   //auto GLOBAL_KMER_temp = (GLOBAL_KMER >> (64-k));
   KmerType temp =  hash ^ GLOBAL_KMER;
   hash = temp;
   //done from randomization.  
